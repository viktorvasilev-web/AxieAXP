...
export function renderAxieCard(row, classColorsOverride = classColors, collectionIcons, viewConfig) {
  ...
  const safeXp = parseInt(row.xpToday) || 0;
  axieCard.setAttribute('data-xp-today', safeXp);

  axieCard.setAttribute('data-delegation', row.Delegation?.toLowerCase() || ""); // ✅ ДОБАВЕНО

  if (typeof row.xpToday === 'number') {
    axieCard.setAttribute('data-xp-today', row.xpToday);
  }

  const displayClassNameMap = {
    Aqua: 'Aquatic'
  };
